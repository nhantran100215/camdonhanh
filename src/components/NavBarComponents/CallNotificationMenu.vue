  ...
  <v-menu offset-y :nudge-left="120" :nudge-bottom="14" :min-width="253">
    <template v-slot:activator="{ on }">
      <v-btn flat icon v-on="on" color="#ffffff" @click="getNotification">
        <v-icon color="#000000">phone_missed</v-icon>
      </v-btn>
    ...
    <v-card>
      <v-list>
        <v-list-tile-title><center style="font-size: 20px;">Call Notifications</center></v-list-tile-title>
        <v-divider></v-divider>
        <v-card class="elevation-0" max-height="250px" style="overflow: auto">
          <v-list two-line class="notificationList">
            <template v-for="(notification, index) in callNotificationResult">
              <v-list-tile
                class="notification"
                :key="index"
                @click="click(index)"
...
export default {
  name: "call-notification-menu",
  computed: {
    ...mapGetters({
      newCallNotificationResult: 'notification/newCallNotificationResult',
      callNotificationResult: 'notification/callNotificationResult',
    }),
    count() {
      return this.newCallNotificationResult.length
    },
  },
  methods: {
      ...mapActions({
      ...
      click(index) {
        const notification = this.callNotificationResult[index]
